...
def unzip_enhanced_xml():
    ...
    try:
        ...
        print(
            f"✗ Extraction Error: Could not find '{xml_filename_inside_zip}' inside the zip file."
        )
    ...
    for event, elem in context:
        if event == "end" and elem.tag == target_tag:
            ...
            if identity is not None:
                entry["name"] = (
                    identity.find(f"{NAMESPACE}name").text
                    if identity.find(f"{NAMESPACE}name") is not None
                    else None
                )
                entry["firstName"] = (
                    identity.find(f"{NAMESPACE}firstName").text
                    if identity.find(f"{NAMESPACE}firstName") is not None
                    else None
                )
                entry["lastName"] = (
                    identity.find(f"{NAMESPACE}lastName").text
                    if identity.find(f"{NAMESPACE}lastName") is not None
                    else None
                )
...
def parse_un_xml(filepath):
    ...
    for individual in root.findall("./INDIVIDUALS/INDIVIDUAL"):
        ...
        entry["name"] = (
            f"{first_name} {second_name} {third_name} {entry['lastName']}".strip()
        )
    ...
    print(
        f"✓ Parsed {len(sanctions_list)} entries ({len(root.findall('./INDIVIDUALS/INDIVIDUAL'))} Inds, {len(root.findall('./ENTITIES/ENTITY'))} Ents)."
    )
    return sanctions_list
