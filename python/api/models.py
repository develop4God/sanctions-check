...
class ScreeningRequest(BaseModel):
    ...
    document_number: Optional[str] = Field(
        default=None, max_length=50, description="Document number (passport, ID, etc.)"
    )
    document_type: Optional[str] = Field(
        default=None, description="Type of document (e.g., 'Passport', 'National ID')"
    )
    ...
    analyst: Optional[str] = Field(
        default=None, description="Analyst name for audit trail"
    )
...
class EntityDetail(BaseModel):
    ...
    all_names: List[str] = Field(
        default_factory=list, description="All known names/aliases"
    )
    aliases: List[str] = Field(default_factory=list, description="Known aliases")
    first_name: Optional[str] = Field(default=None, alias="firstName")
    last_name: Optional[str] = Field(default=None, alias="lastName")
    countries: List[str] = Field(
        default_factory=list, description="Associated countries"
    )
    identity_documents: List[Dict[str, Any]] = Field(
        default_factory=list, description="Identity documents"
    )
...
class MatchDetail(BaseModel):
    ...
    confidence: ConfidenceBreakdownResponse = Field(
    ...
    match_layer: int = Field(
        ..., ge=1, le=4, description="Match layer (1=exact, 2=high, 3=moderate, 4=low)"
    )
    matched_name: str = Field(..., description="Name that matched the query")
    matched_document: Optional[str] = Field(
        default=None, description="Document number that matched (if any)"
    )
...
class ScreeningResponse(BaseModel):
    ...
    matches: List[MatchDetail] = Field(
        default_factory=list, description="Match details"
    )
...
class BulkScreeningResponse(BaseModel):
    ...
    results: List[BulkScreeningItem] = Field(
        default_factory=list, description="Individual screening results"
    )
    processing_time_ms: int = Field(
...
class HealthResponse(BaseModel):
    ...
    data_files: List[DataFileInfo] = Field(
        default_factory=list, description="Data file information"
    )
    data_age_days: Optional[int] = Field(
        default=None, description="Age of data in days (based on oldest file)"
    )
