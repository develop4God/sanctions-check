...
class MatchingConfig:
    ...
    adaptive_thresholds: AdaptiveThresholdConfig = field(
...
class DataConfig:
    ...
    hash_verification: HashVerificationConfig = field(
        default_factory=HashVerificationConfig
    )
...
class ReportingConfig:
    ...
    recommendation_thresholds: Dict[str, int] = field(
        default_factory=lambda: {
            "auto_clear": 60,
            "manual_review": 85,
            "auto_escalate": 95,
...
class ValidationConfig:
    ...
    required_entity_fields: List[str] = field(
        default_factory=lambda: ["id", "name", "source"]
    )
    required_individual_fields: List[str] = field(
        default_factory=lambda: ["id", "name", "source", "type"]
    )
...
class ConfigManager:
    ...
    def _parse_data(self) -> None:
        ...
        self.data = DataConfig(
            ...
            entity_count_variance_threshold=cfg.get(
    ...
    def _parse_reporting(self) -> None:
        ...
        self.reporting = ReportingConfig(
            ...
            recommendation_thresholds=cfg.get(
    ...
    def _parse_validation(self) -> None:
        ...
        self.validation = ValidationConfig(
            required_entity_fields=cfg.get(
                "required_entity_fields", self.validation.required_entity_fields
            ),
            required_individual_fields=cfg.get(
                "required_individual_fields", self.validation.required_individual_fields
            ),
    ...
    def _validate(self) -> None:
        ...
        manual_review = thresholds.get(
            "manual_review", default_thresholds["manual_review"]
        )
        auto_escalate = thresholds.get(
            "auto_escalate", default_thresholds["auto_escalate"]
        )
        ...
        if (
            self.input_validation.name_max_length
            < self.input_validation.name_min_length
        ):
            errors.append(
