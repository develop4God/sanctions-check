...
class SanctionedEntity(Base, TimestampMixin, SoftDeleteMixin):
    ...
    source: Mapped[DataSourceType] = mapped_column(
        Enum(DataSourceType), nullable=False, index=True
    )
    ...
    vessel_imo: Mapped[Optional[str]] = mapped_column(
    ...
    matches: Mapped[List["ScreeningMatch"]] = relationship(
        "ScreeningMatch", back_populates="matched_entity", lazy="dynamic"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "ix_entity_active", "id", postgresql_where=(Column("is_deleted") == False)
        ),
    )
...
class EntityAlias(Base, TimestampMixin):
    ...
    entity: Mapped["SanctionedEntity"] = relationship(
        "SanctionedEntity", back_populates="aliases"
    )
...
class IdentityDocument(Base, TimestampMixin):
    ...
    # Relationship
    entity: Mapped["SanctionedEntity"] = relationship(
        "SanctionedEntity", back_populates="documents"
    )
...
class EntityAddress(Base, TimestampMixin):
    ...
    # Relationship
    entity: Mapped["SanctionedEntity"] = relationship(
        "SanctionedEntity", back_populates="addresses"
    )

    __table_args__ = (Index("ix_address_country_city", "country", "city"),)
...
class EntityFeature(Base, TimestampMixin):
    ...
    # Relationship
    entity: Mapped["SanctionedEntity"] = relationship(
        "SanctionedEntity", back_populates="features"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<EntityFeature(entity_id={self.entity_id}, type='{self.feature_type}')>"
        )
...
class SanctionsProgram(Base, TimestampMixin):
    ...
    entities: Mapped[List["EntityProgram"]] = relationship(
        "EntityProgram", back_populates="program", lazy="dynamic"
    )
...
class EntityProgram(Base, TimestampMixin):
    ...
    # Relationships
    entity: Mapped["SanctionedEntity"] = relationship(
        "SanctionedEntity", back_populates="programs"
    )
    program: Mapped["SanctionsProgram"] = relationship(
        "SanctionsProgram", back_populates="entities"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<EntityProgram(entity_id={self.entity_id}, program_id={self.program_id})>"
        )
...
class ScreeningRequest(Base, TimestampMixin):
    ...
    screened_name: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True, index=True
    )

    # Document number if provided
    screened_document: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, index=True
    )
    ...
    created_at_trunc: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, server_default=None, index=True
    )
...
class ScreeningResult(Base, TimestampMixin):
    ...
    recommendation: Mapped[Optional[RecommendationType]] = mapped_column(
        Enum(RecommendationType), nullable=True
    )
    ...
    request: Mapped["ScreeningRequest"] = relationship(
        "ScreeningRequest", back_populates="results"
    )
    matches: Mapped[List["ScreeningMatch"]] = relationship(
        "ScreeningMatch",
...
class ScreeningMatch(Base, TimestampMixin):
    ...
    document_confidence: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
    dob_confidence: Mapped[float] = mapped_column(Float, nullable=False, default=0.0)
    nationality_confidence: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
    address_confidence: Mapped[float] = mapped_column(
        Float, nullable=False, default=0.0
    )
    ...
    result: Mapped["ScreeningResult"] = relationship(
        "ScreeningResult", back_populates="matches"
    )
    matched_entity: Mapped[Optional["SanctionedEntity"]] = relationship(
        "SanctionedEntity", back_populates="matches"
    )
...
class DataSource(Base, TimestampMixin):
    ...
    updates: Mapped[List["DataUpdate"]] = relationship(
        "DataUpdate", back_populates="source", lazy="dynamic"
    )
...
class DataUpdate(Base, TimestampMixin):
    ...
    # Relationship
    source: Mapped["DataSource"] = relationship("DataSource", back_populates="updates")
