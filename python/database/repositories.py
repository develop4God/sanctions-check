...
class SanctionedEntityRepository:
    ...
    def get_by_external_id(
        self, external_id: str, source: DataSourceType
    ) -> Optional[SanctionedEntity]:
        """
    ...
    def search_by_name(
        self, name: str, threshold: float = 0.3, limit: int = 100
        ...
        id_result = self.session.execute(
            id_query, {"name": normalized, "threshold": threshold, "limit": limit}
        )
        ...
        entity_query = (
            ...
            .where(SanctionedEntity.id.in_(entity_ids))
            .options(
                joinedload(SanctionedEntity.aliases),
                joinedload(SanctionedEntity.documents),
                joinedload(SanctionedEntity.addresses),
    ...
    def search_by_document(
        self, document_number: str, document_type: Optional[str] = None
    ...
    def list_by_source(
        ...
        # Count query
        count_query = (
            select(func.count()).select_from(SanctionedEntity).where(and_(*conditions))
        )
        ...
        query = (
            ...
            .offset(offset)
            .limit(limit)
            .order_by(SanctionedEntity.primary_name)
        )
    ...
    def update(self, entity_id: UUID, updates: Dict[str, Any]) -> SanctionedEntity:
        """
        Update an entity.
    ...
    def count_by_source(
        self, source: Optional[DataSourceType] = None
    ) -> Dict[str, int]:
        """
        ...
        query = (
            ...
            .where(SanctionedEntity.is_deleted == False)
...
class ScreeningRepository:
    ...
    def fail_request(
        self, request_id: UUID, error_message: str, error_code: Optional[str] = None
    ) -> ScreeningRequest:
        """
    ...
    def get_request_with_results(self, request_id: UUID) -> Optional[ScreeningRequest]:
        ...
        query = (
            ...
            .where(ScreeningRequest.id == request_id)
            .options(
    ...
    def get_screening_stats(
        ...
        # Status breakdown
        status_query = select(ScreeningRequest.status, func.count()).group_by(
        ...
        hit_query = (
            ...
            .where(ScreeningResult.is_hit == True)
        )
        if conditions:
