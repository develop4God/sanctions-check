...
from xml_utils import secure_parse, get_secure_parser
...
class SanctionsEntity:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "features": [
                {"type": f.feature_type, "value": f.value, "reliability": f.reliability}
...
class EnhancedSanctionsDownloader:
    ...
    def __init__(self, config: Optional[ConfigManager] = None):
        ...
        self._known_hashes_path = (
            self.data_dir / self.config.data.hash_verification.known_hashes_file
        )
    ...
    def _save_known_good_hash(self, filename: str, file_hash: str) -> None:
        ...
        hashes[history_key].append(
            {"hash": file_hash, "timestamp": datetime.now(timezone.utc).isoformat()}
        )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if hash_config.enabled:
                    is_valid, file_hash = self._verify_hash(
                        filepath, "ofac_enhanced.zip"
                    )
                    ...
                    if not is_valid:
                        if attempt < max_attempts:
                            ...
                            if hash_config.alert_on_mismatch:
                                logger.error(
                                    f"ALERT: OFAC download hash mismatch after {max_attempts} attempts!"
                                )
                                logger.error(
                                    "Download may be corrupted or source file has changed."
                                )
                ...
                logger.error(
                    f"✗ Failed to download OFAC list (attempt {attempt}/{max_attempts}): {e}"
                )
                if attempt < max_attempts:
            ...
            with open(xml_path, "rb") as f:
                ...
                for event, elem in etree.iterparse(f, events=("start",)):
                    ...
                    if tag.startswith("{"):
                        ...
                        if not is_known:
                            logger.warning(
                                f"Namespace differs from expected patterns: {namespace}"
                            )
    ...
    def validate_ofac_xml(
        self, xml_path: Path, xsd_path: Optional[Path] = None
    ) -> ValidationResult:
        ...
        if (
            self.config.data.xsd_validation
            and xsd_path
            and xsd_path.exists()
            and HAS_LXML
        ):
            try:
                ...
                if not schema.validate(xml_doc):
                    ...
                    for error in schema.error_log:
                        ...
                        if xsd_error.severity == "CRITICAL":
                            ...
                            if strictness in ("strict", "normal"):
                                result.add_error(
                                    f"XSD CRITICAL (line {xsd_error.line}): {xsd_error.message}"
                                )
                            else:  # lenient
                                result.add_warning(
                                    f"XSD CRITICAL (line {xsd_error.line}): {xsd_error.message}"
                            ...
                            if strictness == "strict":
                                result.add_error(
                                    f"XSD WARNING (line {xsd_error.line}): {xsd_error.message}"
                                )
                            else:
                                result.add_warning(
                                    f"XSD WARNING (line {xsd_error.line}): {xsd_error.message}"
                            ...
                            if strictness == "strict":
                                result.add_error(
                                    f"XSD INFO (line {xsd_error.line}): {xsd_error.message}"
                                )
                            else:
                                result.add_info(
                                    f"XSD INFO (line {xsd_error.line}): {xsd_error.message}"
                    ...
                    logger.warning(
                        f"XSD validation: {critical_count} critical, {warning_count} warnings, {info_count} info"
                    )

                    if result.is_valid:
                        logger.info(
                            "XSD validation found issues but proceeding (non-critical)"
                        )
                else:
        ...
        try:
            ...
            if HAS_LXML:
                context = etree.iterparse(
                    str(xml_path), events=("end",), tag=f"{ns}entity"
                )
            else:
                ...
                if HAS_LXML or elem.tag == f"{ns}entity":
                    ...
                    if not entity_id:
                        ...
                        if self.config.validation.log_validation_errors:
                            logger.warning(
                                f"Entity missing ID at position {entity_count}"
                            )
            ...
            logger.info(
                f"✓ Validated {entity_count} entities ({malformed_count} malformed)"
            )
    ...
    def _parse_ofac_entity(self, elem: Any, ns: str) -> Optional[SanctionsEntity]:
        ...
        entity_type = (
            entity_type_elem.text if entity_type_elem is not None else "entity"
        )
    ...
    def _parse_feature(self, elem: Any, ns: str) -> Optional[Feature]:
        ...
        feature_type_text = (
            feature_type_elem.text if feature_type_elem.text else "Unknown"
        )
    ...
    def _parse_relationship(self, elem: Any, ns: str) -> Optional[Relationship]:
        ...
        return Relationship(
            ...
            relationship_type=(
                rel_type_elem.text if rel_type_elem is not None else "Unknown"
            ),
    ...
    def parse_un_xml(self, xml_path: Path) -> List[SanctionsEntity]:
        ...
        try:
            ...
            logger.info(
                f"✓ Parsed {len(entities)} UN entities "
    ...
    def _parse_un_individual(self, elem: Any) -> Optional[SanctionsEntity]:
        ...
        name_parts = [
            n for n in [first_name, second_name, third_name, fourth_name] if n
        ]
        if not name_parts:
            return None
    ...
    def _parse_un_reference(
        self,
        dataid: str,
        reference_number: Optional[str],
        un_list_type: Optional[str] = None,
        ...
        if match:
            ...
            if country_code not in self._discovered_country_codes:
                ...
                logger.info(
                    f"Discovered UN country code: {country_code} (from reference: {ref})"
                )
    ...
    def download_and_parse_all(self) -> Tuple[List[SanctionsEntity], ValidationResult]:
        ...
        if self._discovered_country_codes:
            logger.info(
                f"UN country codes discovered: {sorted(self._discovered_country_codes)}"
            )
        if self._discovered_list_types:
            logger.info(
                f"UN list types discovered: {sorted(self._discovered_list_types)}"
            )
