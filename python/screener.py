...
def validate_screening_input(
    input_data: ScreeningInput, config: Optional["ConfigManager"] = None
) -> None:
    ...
    if found_blocked:
        # Log security event
        logger.warning(
            "SECURITY: Blocked characters detected in name input: %s",
    ...
    if iv_config.allow_unicode_names:
        ...
        for char in name:
            ...
            if category.startswith("C"):  # Control or format characters
                logger.warning(
                    "SECURITY: Control character detected in name: %s",
                    sanitize_for_logging(name),
                )
                raise InputValidationError(
        ...
        if not NAME_PATTERN.match(name):
            logger.warning(
                "Invalid name format (Latin-only mode): %s", sanitize_for_logging(name)
            )
            raise InputValidationError(
...
class EnhancedSanctionsScreener:
    ...
    def __init__(
        self, config: Optional[ConfigManager] = None, data_dir: str = "sanctions_data"
    ):
        """Initialize screener
        Args:
        ...
        self._max_history_size = (
            10000  # Limit to prevent memory issues in long-running apps
        )
    ...
    def test_postgres_connection():
        ...
        try:
            ...
            conn.close()
            print(
                f"✅ Conexión exitosa a PostgreSQL ({db.host}:{db.port}) - Resultado: {result}"
            )
            return True
        ...
        logger.info(
            f"   - Short name threshold: {self.config.matching.short_name_threshold}%"
        )
    ...
    def _parse_ofac_entity(self, elem: Any, ns: str) -> Optional[Dict[str, Any]]:
                    ...
                    entity["identity_documents"].append(
                        {
                            ...
                            "number": doc_number,
                            "issuingCountry": self._get_text(
                                doc, f"{ns}issuingCountry"
                            ),
                            "issueDate": self._get_text(doc, f"{ns}issueDate"),
                            "expirationDate": self._get_text(
                                doc, f"{ns}expirationDate"
                            ),
                        }
                    )
            ...
            for feature in features_section.findall(f"{ns}feature"):
                ...
                if feature_type is not None and feature_type.text:
                    ...
                    entity["features"].append(
                        {"type": feature_type.text, "value": value}
                    )
        ...
        if programs is not None:
            program_list = [
                p.text for p in programs.findall(f"{ns}sanctionsProgram") if p.text
            ]
    ...
    def search_by_document(
        self, doc_number: str, doc_type: Optional[str] = None
    ) -> List[MatchResult]:
            ...
            for entity in self._document_index[normalized]:
                ...
                for doc in entity.get("identity_documents", []):
                    if self._normalize_document(doc.get("number", "")) == normalized:
                        if (
                            doc_type is None
                            or doc.get("type", "").upper() == doc_type.upper()
                        ):
    ...
    def search(self, input_data: ScreeningInput, limit: int = 10) -> List[MatchResult]:
        ...
        if input_data.document_number:
            doc_matches = self.search_by_document(
                input_data.document_number, input_data.document_type
            )
        ...
        if is_short:
            ...
            base_threshold, threshold_reason = self._get_adaptive_threshold(
                input_data.name
            )
        ...
        for entity in self.entities:
            ...
            if input_data.document_number:
                ...
                for doc in entity.get("identity_documents", []):
                    if (
                        self._normalize_document(doc.get("number", ""))
                        == input_doc_norm
                    ):
                        doc_score = 100.0
                        matched_doc = doc.get("number")
            ...
            if input_data.date_of_birth and entity.get("dateOfBirth"):
                dob_score = self._calculate_dob_score(
                    input_data.date_of_birth, entity["dateOfBirth"]
                )
            ...
            if is_short:
                ...
                if (
                    threshold_reason != "default"
                    and threshold_reason != "latin_default"
                ):
    ...
    def _generate_reports(
        self, result: Dict[str, Any], matches: List[MatchResult]
    ) -> Dict[str, str]:
        ...
        try:
            ...
            report_files["html"] = generator.generate_html_report(
                screening_result, metadata
            )
            report_files["json"] = generator.generate_json_report(
                screening_result, metadata
            )
...
def main():
    ...
    if csv_path.exists():
        ...
        summary = screener.bulk_screen(
            csv_file=str(csv_path), analyst=None, generate_individual_reports=True
        )
