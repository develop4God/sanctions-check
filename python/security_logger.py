...
class SecurityLogger:
    ...
    def set_request_context(
        self, request_id: Optional[str] = None, user_id: str = "", source_ip: str = ""
    ) -> str:
    ...
    def _sanitize_context(self, context: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for key, value in context.items():
            ...
            safe_key = (
                self._sanitize_input(str(key), max_length=100) if key else "unknown"
            )
            ...
            if value is None:
                ...
                sanitized[safe_key] = [
                    (
                        self._sanitize_input(str(item), max_length=200)
    ...
    def log_xxe_attempt(
        self, source: str = "XML parsing", file_name: str = "", blocked: bool = True
    ) -> None:
...
def get_security_logger(
    log_dir: str = "logs", enable_console: bool = False
) -> SecurityLogger:
    ...
    if _security_logger is None:
        _security_logger = SecurityLogger(
            log_dir=log_dir, enable_console=enable_console
        )
