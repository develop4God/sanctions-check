...
SAMPLE_COUNTRIES = ["US", "GB", "DE", "FR", "ES", "IT", "JP", "CN", "RU", "BR"]
    ...
    class SDNCheckUser(HttpUser):
        ...
        def _warm_up(self):
            ...
            try:
                response = self.client.get(
                    "/api/entities", params={"limit": 10}, name="/api/entities (warmup)"
                )
                if response.status_code == 200:
                    data = response.json()
                    self.entity_ids = [
                        e.get("id") for e in data.get("items", []) if e.get("id")
                    ]
            except Exception:
                pass
        ...
        def search_entity_by_name(self):
            ...
            with self.client.get(
                "/api/search",
                params={"name": name, "threshold": threshold, "limit": 50},
                name="/api/search",
                catch_response=True,
            ) as response:
                if response.status_code == 200:
        ...
        def create_screening_request(self):
            ...
            payload = {
                ...
                "country": (
                    random.choice(SAMPLE_COUNTRIES) if random.random() < 0.3 else None
                ),
            }
