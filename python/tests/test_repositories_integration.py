...
from database.models import DataSourceType, EntityType, AuditAction, DataSource
...
def test_screening_request_and_result(session):
    ...
    request = screening_repo.create_request(
        {"name": "Integration Jane Doe", "document": "X12345"}
    )
...
def test_data_source_and_update(session):
    ...
    update = ds_repo.record_update(
        source_id=ds.id, entities_added=10, entities_updated=5, total_entities=15
    )
...
def test_entity_search_by_name(session):
    ...
    assert (
        len(results) >= 0
    )  # Trigram search may or may not find depending on pg_trgm setup
